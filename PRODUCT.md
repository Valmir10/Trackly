...
2. **Conversation** (discussion lens). A project-scoped chat stream and a per-ticket comment thread, rendered by the *same component*, scoped differently — not two systems that look similar, one system rendered twice. Built in Move 3, frontend/mock-data only.
...
Every object in Trackly explicitly knows and renders its own origin story. A ticket isn't just an isolated entity; it is a live node that visually communicates where it was born (e.g., "Born in Sprint Planning 14 → Decision D-7"), where it was discussed (e.g., "Thread #127"), and what contract obligation it satisfies ("Meridian SOW → Milestone 2"). This bidirectional, clickable lineage is our primary proof of "one object, four surfaces" — a structural capability that scattered incumbent stacks (Slack, Teams, DevOps) cannot replicate without rebuilding from the data layer up.
...
8. **Move 7 — Meetings Surface (Decision Lens): "Minutes that compile."** *Move 4 scale — backend + frontend, not a light frontend pass.* Leverages Move 3's block architecture at document scale: notes render as a `Block[]` inside a document container, same renderer, new container. Introduces a major domain extension — **Decisions as a new first-class addressable entity type**, indexed inside the Move 2 `#` picker framework alongside tickets, which is a real extension of that picker's architecture, not a reuse of it as-is. Three new block kinds: `agendaItem` (self-populating suggestions driven by current board queries — slipping tasks, Pulse warnings, unresolved items rolled forward from the previous meeting), `decision` (keystroke-triggered via `>`, addressable with a deep-linkable ID, the provenance chain's origin point), and `actionItem` (a draft ticket written inline, promoted to the real Board via one keystroke — never a note that exports to a ticket, always the same object from the start). *Guardrail:* if time is tight, cut text-editor complexity or advanced collaborative state before cutting the structural block-to-ticket conversion beat — that conversion is the point of the move, not a nice-to-have on top of it.
9. **Move 8 — Client Room (External Lens) + Milestone Rollup.** *Move 4 scale — heavy backend and security work, not a visual pass.* Implements the external `{ type: 'client', projectId }` scope with strict field-level data filtering — the security model is the filter, not a separate app. Contracts exist solely as a lens: a milestone is a live query aggregating ticket state, and the raw PDF is an object-storage attachment (real file-streaming against MinIO), evidence rather than source of truth. Introduces a second major domain extension — **a ticket dependency graph** (`BlockedBy` relation and cascade-unblock logic in the Domain layer), which has no precedent anywhere earlier in the roadmap and needs its own design pass, not an incidental fit. "Approval as an object": client magic-link authentication (read-plus-approve, scoped per room — full client accounts are explicitly below the line) allows one-click milestone sign-off, which fires a SignalR event into the team stream and clears the downstream tickets gated on that approval. Progress metrics are live queries over board state, zero generated status text — the anti-pattern this whole move exists to avoid.
10. **Move 9 — Cross-Surface Integration & Extended 3-Minute Screenplay.** *Integration pass over Moves 7–8's real objects, not new domain work.* Registers global ⌘K commands searching across the new Decision and Milestone entities, so the palette's reach matches the product's actual surface count. Executes the expanded lifecycle demo screenplay: Client Question → Team Chat → Meeting Decision/Action Item → Board Shift → Live Milestone Rollup → Client Approval → Gated Ticket Unblocks → Provenance Chain Audit — one object traced across all four surfaces in a single continuous scene, closing on its provenance chain in the Ticket Modal. Same craft rule as the 90-second script: exactly one red thing, and it turns green on camera. *Guardrail carries over from Move 7/8: if the finale demo is only as good as its weakest beat, the minimum viable version is one meeting with three block types, one contract with two milestones, one approval that unblocks one ticket — that thin slice still tells the whole story; cut breadth inside a move before cutting a beat's existence.*
