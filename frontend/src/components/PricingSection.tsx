    ...
    <section id="pricing" className="py-24 border-t border-border/40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 text-center">
          <p className="mb-3 text-sm font-medium text-rose-400 uppercase tracking-wider">Pricing</p>
          ...
          {tiers.map((tier) => (
            <div
              ...
              className={`relative flex flex-col rounded-xl border p-8 ${
                tier.highlighted
                  ? 'border-rose-500/60 bg-rose-500/5 shadow-xl shadow-rose-500/10'
                  : 'border-border/60 bg-card'
              }`}
            >
              {tier.badge && (
                <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 bg-rose-600 text-white hover:bg-rose-600 text-xs">
                  {tier.badge}
                </Badge>
              )}
              ...
              <ul className="flex flex-col gap-3">
                {tier.features.map((feature) => (
                  <li key={feature} className="flex items-center gap-2.5 text-sm text-muted-foreground">
                    <Check size={14} className="shrink-0 text-rose-400" />
                    {feature}
                  </li>
                ))}
              </ul>
